            ...
            content: `<!DOCTYPE html>
                        ...
                        <body>
                            <table class="container">
                                ...
                                <tr>
                                    <td class="content">
                                        <h2>Hello, ${newStudent.name}!</h2>
                                        <p>Your account has been successfully created.</p>
                                        <p>Please find your login credentials and user type below:</p>

                                            <div class="info-box">Password: ${studentPassword}</div>
...
const deleteStudent = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const isStudentEnrolled = await Course.findOne({ students: id });
        if (isStudentEnrolled) {
            res.status(HTTP_BAD_REQUEST.code).json({ error: "Student is enrolled in a course" });
            return;
        }
